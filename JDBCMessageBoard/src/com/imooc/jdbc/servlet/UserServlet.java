...
public class UserServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(Objects.equals(pathName, "/userInfo.do")){// 查看用户信息
            request.getRequestDispatcher("/WEB-INF/views/biz/user.jsp").forward(request, response);
        }else if (Objects.equals(pathName, "/editUserPrompt.do")){// 进入编辑用户信息页面
            // 1、从session中获取，其实是与上一个if同样的方式
        ...
        }else if (Objects.equals(pathName, "/editUser.do")){// 修改用户信息
            Long id = Long.valueOf(request.getParameter("id"));
